...
class Plantillas {
	...
	public function show_tpl($view,$data){
		...
		$top['user']['username'] = $this->CI->session->userdata('username');
		$top['user']['name'] = $this->CI->session->userdata('name');
		$top['user']['iduser'] = $this->getUser();
		$top['user']['kindof'] = $this->CI->session->userdata('kindof');
	...
	function set_message($num=1000,$msg=null,$type='error'){
        ...
        if($type=="error")
			file_put_contents(PATHLOG."log_error.txt","[".date('Y-m-d H:i:s')."]:".utf8_encode($code)."\n",FILE_APPEND);
	...
	function show_numErr($num=1000,$txt=null){
		...
		$err[105]='Sus archivos CER y KEY a un no estan vinvualados a su cuenta.';
	...
	public function getUser(){
		return (int) $this->CI->session->userdata('iduser');
	}
	//Verificamos sesion
	public function is_session(){
		$user_id = $this->getUser();
		$emisor = is_array($this->CI->session->userdata('emisor'));
		if (!$user_id){
			...
			if(!($class == "settings" && $user_id>0)){
				$this->set_message('Establesca los parametros de configuracion inicial','warning');
